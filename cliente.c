...
int main(int argc, char *argv[])
{
	...
	while(1) {
		...
		if(!strcmp(escMenu, "1")) {
			...
			board = 0;
			cartelaGerada = false;	
			if (send(Meusocket, "DEL_CARTELA", 12, 0) == -1) {
				perror("Texto Enviado");
				close(Meusocket);
				exit(0);
			}
			...
			printf("\n\nRecebido: %s", buf);
			getchar();
		} else if(!strcmp(escMenu, "5")) {
			char msgm[128];
			fgets(msgm, 128, stdin);
			if (send(Meusocket, msgm, strlen(msgm), 0) == -1) {
				perror("Texto Enviado");
				close(Meusocket);
				exit(0);
			}
...
void printBoard(char * board) {
	char *numero;
	int i, contNumero = 0, contLinhaCartela = 1, contLinhaAtual = 1;
	printf("Sua Cartela Atual:\n\n");
	printf("+----------------------------------+\n|      |      |      |      |      |\n|  ");
	numero = (char*)malloc(2);
	for(i = 0; board[i] != '\0'; i++) {
		if(board[i] != ';') {
			numero[contNumero] = board[i];
			contNumero++;
		} else {
			contNumero = 0;
			if(contLinhaCartela == 3 && contLinhaAtual == 3)
				printf("XX  |  %s  |  ", numero);
			else if(contLinhaCartela == 4 && contLinhaAtual == 3) {
				printf("%s  |  \n|      |      |      |      |      |\n|----------------------------------|\n|      |      |      |      |      |\n|  ", numero);
				contLinhaCartela = 0;
				contLinhaAtual++;
			}
			else if(contLinhaCartela < 5)
				printf("%s  |  ", numero);
			else {
				printf("%s  |  \n|      |      |      |      |      |\n|----------------------------------|\n|      |      |      |      |      |\n|  ", numero);
				contLinhaCartela = 0;
